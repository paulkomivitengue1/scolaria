/*
# Seed default grade_periods in handle_new_school

## Purpose
New schools created via signup now get 3 default grading periods
(Trimestre 1, 2, 3) automatically, matching the report card system.

## Changes
- Appends grade_periods seeding to the existing handle_new_school function.
- No structural table changes, no policy changes.
*/
...
BEGIN
  ...
  -- Seed default grade periods (3 trimesters)
  INSERT INTO public.grade_periods (school_id, period_index, label)
  VALUES
    (v_school_id, 1, 'Trimestre 1'),
    (v_school_id, 2, 'Trimestre 2'),
    (v_school_id, 3, 'Trimestre 3')
  ON CONFLICT (school_id, period_index) DO NOTHING;

  -- Seed fee_config for all classes
  FOREACH v_cls IN ARRAY v_class_names LOOP
    ...
    INSERT INTO public.fee_config (school_id, fee_type, class_name, payment_mode, tranche_index, amount)
    VALUES
      (v_school_id, 'inscription', v_cls, 'single', NULL, 25000),
      (v_school_id, 'frais de fête', v_cls, 'single', NULL, 5000)
    ON CONFLICT (school_id, fee_type, class_name, tranche_index) DO NOTHING;
  ...
  -- Seed old pricing_config for backward compatibility
