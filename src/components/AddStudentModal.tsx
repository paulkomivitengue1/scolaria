...
import { X, UserPlus, GraduationCap, FileText, PartyPopper, Utensils, Bus, Check, Layers } from 'lucide-react';
import type { Student, FeeTypeDef, FeeConfigRow, FeeSubscription, TrancheDef } from '../types';
import { CLASS_LIST, formatFCFA, getFeeTotalForClass, FEE_TYPE_ICONS, emptyTranchePayments, emptySinglePayment } from '../types';
  ...
  const submit = () => {
    ...
    const fees: FeeSubscription[] = selectedFeeTypes.map(f => {
      ...
      const payments: Record<string, { paid: number }> = f.paymentMode === 'tranche'
        ? emptyTranchePayments(tranches.length)
        : emptySinglePayment();
